...
export function canPlay(state: UnoState, card: UnoCard): boolean {
  if (state.pendingDraw > 0) {
    if (state.pendingType === 'draw2') return state.rules.stackDraw2 && card.value === 'draw2';
    if (state.pendingType === 'wild4') return state.rules.stackDraw4 && card.value === 'wild4';
    return false;
  }
...
export function legalPlays(state: UnoState, playerId: string): number[] {
  ...
  if (idx !== state.turn) {
    ...
    const top = state.discard[state.discard.length - 1] as UnoCard;
    return player.hand
      .filter((c) => c.color !== null && c.color === top.color && c.value === top.value)
      .map((c) => c.id);
  }
