...
function endRound(state: UnoState, winnerIdx: number): void {
  ...
  for (const p of state.players)
    if (p.id !== winner.id) points += p.hand.reduce((s, c) => s + cardPoints(c), 0);
  winner.score += points;
  ...
  if (state.rules.targetScore !== null && winner.score < state.rules.targetScore) {
...
function applyEffect(state: UnoState, actorIdx: number, card: UnoCard): void {
  ...
  if (card.value === 'skip') {
    ...
    state.phase = 'chooseColor';
  } else if (state.rules.sevenZero && card.value === 7 && actor.hand.length > 0) {
...
export function playCard(
  ...
  if (isJumpIn) {
    ...
    if (card.color === null || card.color !== top.color || card.value !== top.value)
    ...
    if (!canPlay(state, card)) throw new Error('Эту карту сейчас нельзя сыграть');
  }
