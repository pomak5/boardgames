...
- **CI** (`.github/workflows/ci.yml`, на каждый PR и push в main) — две джобы:
  - `check`: `bun install --frozen-lockfile` → `lint` → `format:check` (prettier)
    → `typecheck` → `bun test`. Покрывает `apps/server` + `packages/*`.
  - `web`: `apps/web` — biome `check` + `typecheck` + `build`. Перед пушем фронта
    прогоняй `bun run format` (biome `--write`) локально, иначе джоба красная.
