  ...
  test("раздача и первая карта не +4", () => {
    for (let i = 0; i < 20; i++) {
      ...
      for (const p of s.players)
        expect(p.hand.length).toBeGreaterThanOrEqual(7);
...
describe("вариации правил", () => {
  ...
  test("7-0: семёрка меняет руки, ноль крутит по кругу", () => {
    ...
    expect(
      (s3.players[0] as { hand: UnoCard[] }).hand.map(x => x.value),
    ).toEqual([2]);
    expect(
      (s3.players[2] as { hand: UnoCard[] }).hand.map(x => x.value),
    ).toEqual([1]);
    ...
    expect(
      (s5.players[1] as { hand: UnoCard[] }).hand.map(x => x.value),
    ).toEqual([1]);
    expect(
      (s5.players[2] as { hand: UnoCard[] }).hand.map(x => x.value),
    ).toEqual([1]);
    expect(
      (s5.players[0] as { hand: UnoCard[] }).hand.map(x => x.value),
    ).toEqual([2]);
  });
...
describe("UNO! и финал", () => {
  ...
  test("окно поимки закрывается после хода следующего", () => {
    ...
    const s3 = playCard(
      s2,
      "p1",
      (s2.players[1] as { hand: UnoCard[] }).hand[0]?.id as number,
    );
