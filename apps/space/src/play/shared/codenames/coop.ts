...
export function createCoopGame(
  ...
  const state = createGame(words, { startingTeam: playerTeam, random: options.random });
  return { state, playerTeam, timerTeam: otherTeam(playerTeam), cluesUsed: 0, trace: null };
}
...
function timerReveal(g: CoopGame, random: () => number): CoopGame {
  ...
  const cards = s.cards.map((c, i) => (i === pick.i ? { ...c, revealed: true } : c));
  ...
  const state: CodenamesState = {
    ...
    log: [
      ...s.log,
      { type: 'guess', team: g.timerTeam, cardIndex: pick.i, owner: g.timerTeam },
      ...(finished
        ? [{ type: 'gameover' as const, winner: g.timerTeam, reason: 'all-words' as const }]
        : []),
    ],
  };
  return { ...g, state };
...
export function coopGiveClue(g: CoopGame, risk: BotRisk = 'normal'): CoopGame {
  if (g.state.phase === 'finished') throw new CodenamesError('GAME_FINISHED', 'Игра окончена');
  if (g.state.phase !== 'clue') throw new CodenamesError('WRONG_PHASE', 'Сейчас ход игроков');
  const trace = suggestClue(g.state, g.playerTeam, risk);
  if (!trace) throw new CodenamesError('INVALID_CLUE_WORD', 'Бот не нашёл подсказку');
  return { ...g, state: giveClue(g.state, trace.clue), cluesUsed: g.cluesUsed + 1, trace };
}
...
export function coopPass(g: CoopGame, random: () => number = Math.random): CoopGame {
...
export function coopResult(g: CoopGame): CoopResult | null {
  ...
  const margin = g.state.cards.filter((c) => c.owner === g.timerTeam && !c.revealed).length;
  return { won: g.state.winner === g.playerTeam, margin, cluesUsed: g.cluesUsed };
}
