...
type TileCard = Pick<Card, "word" | "revealed"> & {
...
function KeyStamp({ owner }: { owner: Exclude<Card["owner"], null> }) {
  return (
    <svg
      className={`cn-stamp cn-stamp--${owner}`}
      viewBox="0 0 24 24"
      aria-hidden
    >
      <circle
      ...
      {owner === "assassin" ? (
        ...
        <path
          d="M12 5.8l1.7 3.7 4 .5-3 2.8.8 4-3.5-2-3.5 2 .8-4-3-2.8 4-.5z"
          fill="currentColor"
        />
      )}
    </svg>
...
export function CardTile({ card, disabled, spymasterView, onReveal }: Props) {
  ...
  const dimClass =
    spymasterView && !card.revealed && card.owner
      ? `cn-key--${card.owner}`
      : "";
  return (
    <button
