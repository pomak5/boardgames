...
import { ImaginariumHome } from "./ImaginariumHome";
import { ImaginariumLobby } from "./ImaginariumLobby";
import { ImaginariumTable } from "./ImaginariumTable";
import { useImaginariumRoom } from "./useImaginariumRoom";

const PHASE_LABEL: Record<string, string> = {
  association: "Ведущий загадывает",
  choosing: "Выбор карт",
  voting: "Голосование",
  scoring: "Результаты",
...
/** Базовый путь экрана Имаджинариума (без кода комнаты). */
const BASE = "/imaginarium";

export function ImaginariumApp() {
  const [settingsOpen, setSettingsOpen] = useState(false);
  const [copied, setCopied] = useState(false);
  const api = useImaginariumRoom();
  const auth = useAuth();
  ...
  // Синхронизируем URL с текущей комнатой: /imaginarium/CODE в комнате,
  // /imaginarium — на входе.
  useEffect(() => {
  ...
  // Авто-вход по ссылке /imaginarium/CODE: если ник известен — заходим сразу,
  // иначе код подставится в поле на экране входа.
  ...
  const statusText = !room
    ...
    : room.phase === "lobby"
      ...
      : room.phase === "finished"
        ? game?.winner?.length
          ? `Победа: ${game.winner.join(", ")}`
          : "Партия завершена"
        : game
          ? (PHASE_LABEL[game.phase] ?? "Партия")
          : "Партия";
