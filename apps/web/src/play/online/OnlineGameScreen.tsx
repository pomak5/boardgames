...
function BotIcon({ size = 18, className }: { size?: number; className?: string }) {
  return (
...
/** Аватар игрока: настоящая картинка из профиля либо инициалы. */
function Avatar({ nickname, avatarUrl, size = 36 }: { nickname: string; avatarUrl?: string | null; size?: number }) {
  return <ProfileAvatar nickname={nickname} avatarUrl={avatarUrl ?? null} size={size} />;
}
...
const SEAT_BTN =
  "text-xs font-semibold px-3 h-8 rounded-lg border border-game-border bg-white hover:bg-gray-50 transition-colors active:translate-y-[1px] inline-flex items-center gap-1.5";
...
function TeamPanel({
  ...
  return (
    <div className={`flex flex-col gap-4 w-full md:w-[260px] ${isRed ? "md:items-start" : "md:items-end"}`}>
        ...
        <div className="w-full rounded-3xl border border-game-border bg-white p-5 flex flex-col shadow-sm">
          <div>
            ...
            <div className="flex items-center gap-3">
              <Avatar nickname={captain ? captain.nickname : botCaptain ? "Бот" : "?"} avatarUrl={captain?.avatarUrl} size={48} />
            ...
            {canEditSeats && (
              <div className="flex flex-wrap gap-2 mt-3">
                {captain ? (
                  iAmCaptain && (
                    <button type="button" className={SEAT_BTN} onClick={() => api.setCaptain(team, "open")}>
                      Встать
                    </button>
                  )
                ) : botCaptain ? (
                  ...
                  <>
                    <button type="button" className={SEAT_BTN} onClick={() => api.setCaptain(team, "me")}>
                      <SpyIcon size={14} /> Сесть мастером
                    </button>
                    <button type="button" className={SEAT_BTN} onClick={() => api.setCaptain(team, "bot")}>
                      <BotIcon size={14} /> Бот
                    </button>
                  </>
                )}
          ...
          <div>
            ...
            <div className="flex flex-col gap-3.5">
              {agents.map((agent) => (
                <div key={agent.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <Avatar nickname={agent.nickname} avatarUrl={agent.avatarUrl} size={36} />
                    <span className="text-base font-medium text-game-text/90">
                      {agent.nickname}
                      {agent.id === me?.id ? " — вы" : ""}
                    </span>
                  </div>
            ...
            {canEditSeats && !iAmGuesserHere && (
              <button type="button" className={`${SEAT_BTN} mt-3`} onClick={() => api.setTeam(team, "guesser")}>
                Войти отгадывающим
              </button>
            )}
          </div>
...
function ActionPanel({
  ...
  if (!me.team) {
    return (
      <div className="w-full max-w-[800px] mx-auto mt-6 text-center text-game-text/60 font-medium">
        Выберите команду в колонках по бокам — отгадывающим или мастером.
      </div>
    );
  }

  const roleLabel = iClue
    ? `Вы — шпион-мастер ${TEAM_RU_GEN[me.team]}`
    : iGuess
      ? `Вы — агент ${TEAM_RU_GEN[me.team]}`
      : "";
...
function BottomPanels({ api, game, howtoRef }: { api: RoomApi; game: CodenamesView; howtoRef: RefObject<HTMLDivElement | null> }) {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6 w-full mt-6 md:h-64">
      <div className="flex-1 rounded-2xl border border-game-border bg-game-panel shadow-panel p-5 flex flex-col min-h-[16rem]">
          ...
          {api.chat.map((m) => {
            ...
            return (
              <div key={`${m.sentAt}-${m.authorId}`} className="text-sm flex gap-3 items-start">
                <Avatar nickname={m.authorName} avatarUrl={author?.avatarUrl} size={24} />
      ...
      <div ref={howtoRef} className="w-full md:w-80 rounded-2xl border border-game-border bg-game-panel shadow-panel p-5 flex flex-col">
      ...
      <header className="cg-top">
        ...
        <div className="cg-top__actions">
          ...
          <span className="cg-me" title={me.nickname}>
            <Avatar nickname={me.nickname} avatarUrl={me.avatarUrl} size={36} />
          </span>
        </div>
      </header>

      <main className="max-w-[1400px] mx-auto px-2 mt-4 flex flex-col md:flex-row md:items-start gap-6 md:gap-8" style={cgStyle}>
        <TeamPanel
          team="red"
        ...
        <div className="flex-1 flex flex-col min-w-0 pb-8 order-first md:order-none">
