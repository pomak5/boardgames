...
import "./alias.css";
import { AliasHome } from "./AliasHome";
import { AliasLobby } from "./AliasLobby";
import { AliasTable } from "./AliasTable";
import { useAliasRoom } from "./useAliasRoom";

const TEAM_LABEL: Record<string, string> = { red: "Красные", blue: "Синие" };

/** Базовый путь экрана Элиас (без кода комнаты). */
const BASE = "/alias";

export function AliasApp() {
  const [settingsOpen, setSettingsOpen] = useState(false);
  const [copied, setCopied] = useState(false);
  const api = useAliasRoom();
  const auth = useAuth();
  ...
  // Синхронизируем URL с текущей комнатой: /alias/CODE в комнате, /alias — на входе.
  useEffect(() => {
  ...
  // Авто-вход по ссылке /alias/CODE (как в Коднеймс): если ник известен — заходим
  ...
  const statusText = !room
    ...
    : room.phase === "lobby"
      ...
      : room.phase === "finished"
        ? game?.winner
          ? `Победа: ${TEAM_LABEL[game.winner] ?? game.winner}`
          : "Партия завершена"
        : game
          ? game.phase === "round"
            ? `Объясняет: ${TEAM_LABEL[game.currentTeam] ?? game.currentTeam}`
            : "Между раундами"
          : "Партия";
  ...
  return (
    <div className="al-app">
      ...
      {inRoom && room.phase === "lobby" && <AliasLobby api={api} />}
      {inRoom && room.phase !== "lobby" && <AliasTable api={api} />}
